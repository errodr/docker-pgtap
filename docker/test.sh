...
if [[ $VERBOSE_TESTS == 1  ]] && [[ $JUNIT == 1 ]] ; then
    JUNIT_OUTPUT_FILE=$JUNIT_TEST_RESULTS PGPASSWORD=$PASSWORD pg_prove -h $HOST -p $PORT -d $DATABASE -U $USER -v --harness TAP::Harness::JUnit $TESTS
